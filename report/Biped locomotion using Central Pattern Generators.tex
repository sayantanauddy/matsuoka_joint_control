...
\newcommand{\trtitle}{Bio-inspired Bipedal Locomotion}
...
\begin{figure}[htbp]
\centering     %%% not \center
\subfigure[Oscillation of the shoulder joints with different frequencies]{\label{fig:nao_motion}\includegraphics[scale=0.25]{figures/nao_joint_motion.png}}
\subfigure[Two joints moving in-phase but with different frequencies. Coupling is switched on at t=10s]{\label{fig:nao_graph}\includegraphics[scale=0.35]{figures/double-joint-in-phase-diff-freq.eps}}
\caption{Coupled joint oscillations on the robot}
